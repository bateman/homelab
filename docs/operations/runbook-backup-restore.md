...
| Source size >500 MB | Missing exclusion filters for regenerable data | Verify all 24 glob filters from step 4 are applied (all "Exclude expression"). Expected source: ~400 MB or less (dominated by lidarr.db 196 MB + gravity.db 83 MB + Plex metadata + other *arr DBs + config files) |
...
| Date | Change |
|------|--------|
| 2026-03-17 | Expanded to 24 glob filters — added plex-music exclusions (Crash Reports, Codecs); added *arr (UpdateLogs, asp), qBittorrent (GeoDB, nova3), Uptime Kuma (kuma.db.bak\*, screenshots). Rejected Definitions/ (would exclude Prowlarr custom indexers) and Backup/ (Plex doesn't use a top-level Backup dir; risk of matching Bazarr's backup/) |
